...
def parse_sha256sums(text: str) -> dict[str, str]:
    ...
    for line_number, raw_line in enumerate(text.splitlines(), start=1):
        ...
        if len(parts) != 2 or len(digest) != 64 or any(char not in "0123456789abcdef" for char in digest):
...
def resolve_expected_sha256(deploy_dir: Path, filename: str, override: str | None,
                            metadata: Mapping | None = None) -> tuple[str, str]:
    if override and override.strip():
        digest = override.strip().lower()
        source = "environment override"
    else:
        sums_path = deploy_dir / "SHA256SUMS.txt"
        sums = parse_sha256sums(sums_path.read_text(encoding="utf-8")) if sums_path.is_file() else {}
...
class OnnxSemanticEngine:
    ...
    def __init__(self, deploy_dir: Path, expected_sha256: str | None = None, verify_sha256: bool = True):
        ...
        if verify_sha256:
            expected, self.checksum_source = resolve_expected_sha256(deploy_dir, self.model_path.name, expected_sha256, self.metadata)
            if self.model_sha256 != expected:
                raise RuntimeError(f"Semantic ONNX SHA256 mismatch ({self.checksum_source})")
        ...
        if len(outputs) != 1 or outputs[0].name != "output0" or outputs[0].type not in {"tensor(int64)", "tensor(float)"} or len(outputs[0].shape) != 3:
        ...
        warmup = self.session.run([self.output_name], {self.input_name: np.zeros((1, 3, height, width), dtype=np.float32)})[0]
    ...
    def _validate_metadata(self) -> None:
        ...
        if meta.get("task") != "semantic-segmentation" or input_meta.get("layout") != "NCHW" or input_meta.get("color") != "RGB" or input_meta.get("dtype") != "float32" or input_meta.get("scale") != "0..1":
        ...
        if output_meta and (output_meta.get("name"), output_meta.get("layout"), output_meta.get("semantics")) != ("output0", "NHW", "public-class-id-map"):
    ...
    def infer(self, tensor: np.ndarray) -> tuple[np.ndarray, int]:
        ...
        if tensor.shape != (1, 3, height, width) or tensor.dtype != np.float32:
            raise RuntimeError(f"Unexpected semantic input tensor {tensor.shape}/{tensor.dtype}")
