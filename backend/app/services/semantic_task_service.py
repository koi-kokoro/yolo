...
from app.entity.db_models import DetectionScene, ModelVersion, SemanticResult, SemanticTask
...
class SemanticTaskService:
    ...
    def _default_model(self, db: Session) -> ModelVersion:
        model = db.query(ModelVersion).filter(ModelVersion.task_kind == "semantic_segmentation", ModelVersion.is_default.is_(True), ModelVersion.status == "active").first()
        if not model:
    ...
    def check_create_allowed(self, db: Session, user_id: int) -> None:
        ...
        active = db.query(SemanticTask).filter(SemanticTask.user_id == user_id, SemanticTask.status.in_(("pending", "running"))).count()
    ...
    def create(self, db: Session, user_id: int, validated) -> SemanticTask:
        ...
        task = SemanticTask(task_uuid=task_uuid, user_id=user_id, model_version_id=model.id, status="pending", original_filename=validated.original_filename, source_object_key=key, source_sha256=validated.sha256, source_content_type=validated.content_type, image_width=validated.width, image_height=validated.height)
        try:
            db.add(task)
            db.commit()
            db.refresh(task)
        ...
        if not semantic_runtime.submit(self.process, task.id):
            task.status, task.error_code, task.error_message, task.completed_at = "failed", "INFERENCE_QUEUE_FULL", "任务提交失败，请稍后重试", datetime.now()
            db.commit()
        return task
    ...
    def process(self, task_id: int) -> None:
        ...
        try:
            claimed = db.execute(update(SemanticTask).where(SemanticTask.id == task_id, SemanticTask.status == "pending").values(status="running", started_at=datetime.now())).rowcount
            db.commit()
            if claimed != 1:
                return
            task = db.query(SemanticTask).options(joinedload(SemanticTask.model_version)).filter(SemanticTask.id == task_id).one()
            import io
            ...
            keys = [f"{prefix}/index_mask.png", f"{prefix}/color_mask.png", f"{prefix}/overlay.png"]
            ...
            if task and task.status not in ("succeeded", "failed"):
                task.status, task.error_code, task.error_message, task.completed_at = "failed", "INFERENCE_FAILED", "语义分割处理失败", datetime.now()
                db.commit()
    ...
    def get_owned(self, db: Session, task_uuid: str, user_id: int) -> SemanticTask:
        task = db.query(SemanticTask).options(joinedload(SemanticTask.model_version), joinedload(SemanticTask.result)).filter(SemanticTask.task_uuid == task_uuid, SemanticTask.user_id == user_id).first()
        if not task:
            raise DomainError(404, "SEMANTIC_TASK_NOT_FOUND", "任务不存在")
        return task

    def list_owned(self, db: Session, user_id: int, page: int, page_size: int, status: str | None):
        query = db.query(SemanticTask).options(joinedload(SemanticTask.model_version), joinedload(SemanticTask.result)).filter(SemanticTask.user_id == user_id)
        if status:
        ...
        total = query.count()
        items = query.order_by(SemanticTask.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
        return total, math.ceil(total / page_size) if total else 0, items

    @staticmethod
    def recover_interrupted(db: Session) -> int:
        count = db.query(SemanticTask).filter(SemanticTask.status.in_(("pending", "running"))).update({"status": "failed", "error_code": "SERVICE_RESTARTED", "error_message": "服务已重启，请重新创建任务", "completed_at": datetime.now()}, synchronize_session=False)
        db.commit()
        return count
