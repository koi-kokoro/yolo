...
def create_report_tools(user_id: int) -> list[Any]:
    ...
    @tool
    def collect_patrol_report_data(days: int = 30, domain: str = "all") -> str:
        """收集当前用户指定周期内的巡查统计、趋势、类别分布和近期任务。"""
        safe_days = max(1, min(int(days), 365))
        scene_name = "dior_facility_detection" if domain == "dior" else None
        ...
        if scene_name:
            history_kwargs["scene_name"] = scene_name
        recent = history_service.list_tasks(**history_kwargs)
        ...
        statistics = (
            dashboard_service.get_statistics(
                user_id=user_id, days=safe_days, scene_name=scene_name
            )
            if scene_name
        ...
        history_summary = (
            history_service.get_summary(user_id=user_id, scene_name=scene_name)
            if scene_name
