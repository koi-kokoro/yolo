...
class TrainingService:
    ...
    def _run_training(task_id: int, task_uuid: str, config: dict):
        ...
        try:
            ...
            # ── 加载预训练模型 ──
            model_name = config.get("model_name") or "yolov11n"
            model_files = {
                "yolov11n": "yolo11n.pt",
                "yolov11s": "yolo11s.pt",
                "yolov11m": "yolo11m.pt",
            ...
            model_file = model_files.get(model_name, model_name)
            logger.info("加载预训练模型：%s（首次使用将自动下载）", model_file)
            ...
            try:
                if not data_yaml:
                    raise ValueError("data.yaml 路径尚未初始化")
                with open(data_yaml, "w", encoding="utf-8") as f:
    ...
    def get_training_status(db, task_id: int) -> dict:
        ...
        Returns:
            状态字典，包含：
                ...
                - is_running: 是否在运行中
        """
        task: Any = db.query(TrainingTask).filter(TrainingTask.id == task_id).first()
        if not task:
