...
class DatasetSplitter:
    ...
    def generate_data_yaml(
        ...
        Args:
            output_dir: YOLO 数据集根目录。
            class_names: 类别英文名称列表，顺序即类别 ID。
            class_names_cn: 可选中文类别名，仅写入注释，方便查看。
            yaml_name: 输出 YAML 文件名。
        ...
        for idx, name in enumerate(class_names):
            comment = ""
            if class_names_cn and idx < len(class_names_cn):
                comment = f"  # {class_names_cn[idx]}"
            lines.append(f"  {idx}: {name}{comment}")
